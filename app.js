...
document.getElementById('shrinkageForm').addEventListener('submit',function(e){ 
  ...
  flds.forEach(f=>{ 
    ...
    let s=(!isNaN(b)&&!isNaN(a)&&b>0)?((b-a)/b*100).toFixed(5):'';  // 5 decimals here
...
// Fit converter (with Length added, 5 decimals) 
document.getElementById('fitForm').addEventListener('submit',function(e){ 
  e.preventDefault(); 
  haptic([28,35,28]); 
  let r=''; 
  const fitFields=[ 
    {label:'Length', before:'lengthBeforeFit', after:'lengthAfterFit'},  // Length added here
  ...
  fitFields.forEach(f=>{ 
    const b=parseFloat(document.getElementById(f.before).value); 
    const a=parseFloat(document.getElementById(f.after).value); 
    let diff=(!isNaN(b)&&!isNaN(a))?(a-b).toFixed(5):'';  // 5 decimals here
